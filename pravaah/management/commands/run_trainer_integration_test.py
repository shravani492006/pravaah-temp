...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if not logged_in:
                self.stderr.write('ERROR: Could not log in as created test user')
                return 1

            resp = client.get('/trainers/profile/')
            if resp.status_code != 200:
                self.stderr.write(f'ERROR: trainers/profile returned status {resp.status_code}')
                return 2
